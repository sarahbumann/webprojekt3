...
$errors = [];
$qry = 'SELECT * FROM `film` WHERE `id` = :id';
$stmt = $pdo->prepare($qry);
$stmt->execute([':id' => $_REQUEST['id']]);
$_POST = array_merge($stmt->fetch(PDO::FETCH_ASSOC), $_POST);
if (isset($_POST['submit']) && !empty($_POST['submit'])) {
	...
	if (empty($errors)) {
		unset($_POST['submit']);
		$qry  = 'REPLACE INTO `film`(`' . implode('`, `', array_keys($_POST)) . '`) VALUES (:' . implode(', :', array_keys($_POST)) . ')';
		$stmt = $pdo->prepare($qry);
		$data = [];
